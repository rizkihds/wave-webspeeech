    ...
    <div class="flex space-x-2 sm:space-x-6">
        <aside class="flex-shrink-0 space-y-2 sm:space-y-6 lg:w-64">
            <x-card class="py-2 sm:py-6">
                <h3 
                    class="hidden px-6 pb-3 text-xs 
                           font-semibold leading-4 tracking-wider 
                            uppercase text-zinc-500 md:block">Actions</h3>
                <nav class="space-y-1">
                    <x-settings-sidebar-link :href="$urls" 
                        icon="{{$iconz}}">{{ $prevmessage }}</x-settings-sidebar-link>
                </nav>
            </x-card>
        </aside>
